...
function check() {
    const  yamlex = require('../es6/YamlTest');
    const yaml_global = yaml.safeLoad(fs.readFile('./GLOBAL.yaml', 'utf8'));
    const yaml_local = yaml.safeLoad(fs.readFile('./HAT_SAAS.yaml', 'utf8'));

    const yaml_attr = {...yaml_global['ATTR'], ...yaml_local['ATTR']};
    const yaml_model = {...yaml_global['MODEL'], ...yaml_local['MODEL']};

    for (let i in yaml_model) {
        if (yaml_model[i]['TYPE'] === 'STREE')
            yaml_stree[i] = yaml_model[i];
        else if (yaml_model[i]['TYPE'] === 'EVENT')
            yaml_event[i] = yaml_model[i];
    }
    return function* () {
        var urlParam=this.url.split('/');
        ...
        if (project !== yaml_local['PROJECT']['KEY']) {
            this.restql.response.status(404).send('Project key not found');
            yield* next
        }

        if (event&&!(event in yaml_event)) {
            this.restql.response.status(404).send('Event not found');
            yield* next
        }

        let query = Object.keys( this.restql.request.query);
        if (query.length !== 1) {
            res.status(400).send('Argument(s) not correct');
            yield* next
        }
        let event1 = yaml_event[req.params.event];
        let attr_key = query[0];
        if (!event1['ATTR-LIST'].includes(attr_key)) {
            this.restql.response.status(400).send('Attribute not exists');
            yield* next
        }

        let attr = yaml_attr[attr_key];
        ...
        switch (attr['TYPE']) {
            ...
            case 'STRING':
                ...
                break;
            case 'MODEL':
                let stree_values = this.url.split('/');
                let model = yaml_stree[attr['MODEL']];
                let stree = model['STREE'];
                let attr_list = model['ATTR-LIST'];
                let level = stree;
                if (stree_values.length > attr_list.length) {
                    this.restql.response.status(400).send('STREE too long');
                    yield* next;
                }
                for (let i = 0; i < stree_values.length; i++) {
                    if (!(stree_values[i] in level)) {
                        this.restql.response.status(400).send('STREE value does not match');
                        yield* next;
                    }
                    level = level[stree_values[i]];
                }

                values  = stree_values;
                break;
            default:
                this.restql.response.status(400).send('Invalid attribute type');
...
function before() {
    ...
    return function* (next) {
        ...
        // TODO 此处添加对url的验证
        console.log(this.url+'urlParam:'+urlParam);
...
function pagination(model) {
    return function* (next) {
        ...
        //当前数量
        response.headers['count']=rows.length;
        response.body = rows
