...
function check() {
    ...
    return function* () {
        ...
        if (project !== yaml_local['PROJECT']['KEY']) {
            ...
            this.response.body='Project key not found';
        ...
        if (event&&!(event in yaml_event)) {
            this.response.status =  404
            this.response.body='Event not found';
            return;
        }
        ...
        if (query.length !== 1) {
            ...
            this.response.body='Argument(s) not correct';
            return;
        ...
        if (!event1['ATTR-LIST'].includes(attr_key)) {
            this.response.status =  400
            this.response.body='Attribute not exists';
            return;
        }
        ...
        switch (attr['TYPE']) {
            ...
            case 'MODEL':
                ...
                if (stree_values.length > attr_list.length) {
                    this.response.status =  400
                    this.response.body='STREE too long';
                    return;
                ...
                for (let i = 0; i < stree_values.length; i++) {
                    if (!(stree_values[i] in level)) {
                        this.response.status =  400
                        this.response.body='STREE value does not match';
                        return;
                    }
            ...
            default:
                this.response.status =  400
                this.response.body='Invalid attribute type';
                return;
